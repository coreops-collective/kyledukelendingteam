/**
 * POST { event_type, context, callerEmail } — emails all matching
 * notification_rules. Inlined helpers (see save-email-delivery for
 * rationale).
 *
 * Auth (S2): the caller must (a) come from an allowed Origin/Referer
 * and (b) present a callerEmail that matches a row in the users table.
 * Rate-limited via public.rate_limit_bump (S5). Fail-open on rate-limit
 * lookup failures so notifications keep flowing during Supabase blips.
 */
...
// A logged-in user (any role) is fine — this endpoint only sends to
// email addresses derived from notification_rules, so an authenticated
// caller can't redirect mail to arbitrary recipients.
...
exports.handler = async (event) => {
  ...
  if (!(await checkRateLimit(event, 'send-notification', 30))) {
    return { statusCode: 429, headers: corsHeaders, body: JSON.stringify({ error: 'Rate limit exceeded' }) };
  }

  try {
    const { event_type, context = {}, callerEmail } = JSON.parse(event.body || '{}');
